...
#include<stdlib.h>    // dynamic memoy allocation (malloc , free)
#include<string.h>    // for string inbuild function
...
#define MAXINODE 50   // Inode number

#define READ 1
#define WRITE 2    // permitions for file
// Execute
#define MAXFILESIZE 1024

#define REGULAR 1   // File types(txt)
#define SPECIAL 2   // ----------  update 

#define START 0    //offset
#define CURRENT 1  //(Lseek)
#define END 2
...
typedef struct inode
{
    ...
    int FileSize;
    int FileActualSize;
    int FileType;                //  Linklist
    char *Buffer;
    int LinkCount;
    int ReferenceCount;
    int Permission; // 1   23
    struct inode *next;         // self refriential structure
...
////////  Globle veriable's  //////////////////
// onject's of structure
UFDT UFDTArr[50];          //Array
SUPERBLOCK SUPERBLOCKobj;  // object
PINODE head = NULL;        // ( shishakachya dokyatli value )Linklist pointer
/////////////////////////////////////////////////
...
void InitialiseSuperBlock()     //  (1) 
{                         // he function UFDTArr chya array madhey NULL value set karel
    int i = 0;
    while(i < MAXINODE)
    {
    ...
    SUPERBLOCKobj.TotalInodes = MAXINODE;     // writesize var kopra 'SuperBlockobj'
    SUPERBLOCKobj.FreeInode = MAXINODE;       // set super block
}
...
int main()
{
    // atoi : ascii to integer karnare function 
    //  important 3 : 42  update code

    char *ptr = NULL;    
    int ret = 0, fd = 0, count = 0;
    char command[4][80], str[80], arr[1024]; //it a 2 dimentinal array which contain

    InitialiseSuperBlock(); // Oxzilary data initialisation    line : 221
    CreateDILB();          //  Line : 186
    ...
    {
        fflush(stdin);    // Scanf(%[^'\n']s)     (juna data clear)
        strcpy(str,"");   // Inbuilt funtion 
        
        printf("\n Marvellous VFS : >");

        fgets(str,80,stdin);   //scanf("%[^'\n']s",str);  fgets(kashat = str , kiti = 80 letters ,kuthun = stdin(keybord))
        
        
       // sscanf = string scanf
       // count = sscanf(khutun = str,"(thyala 4 tukdyat todun dya "%s %s %s %s")%s %s %s %s",command[0],command[1],command[2],command[3]);
        // 3 : 23 min 
        ...
        if(count == 1)
        {
            ...
            else if(strcmp(command[0],"exit") == 0)
            {
                ...
                // free() malloc 
                // UFDT -> Filetable -> IIT and inside iit there are filename is present  
                break;
            }
            else
            {
        ...
        else if(count == 2)//////////////////////////////////////////////////////////////////////////////////
         {
                ...
                else if(strcmp(command[0],"fstat") == 0)
                {
                    ...
                    if(ret == -1)
                       printf("ERROR : Incorrent parameters \n");
                    if(ret == -2)
                       printf("ERROR : There is no such file \n");
                    continue;
                }
